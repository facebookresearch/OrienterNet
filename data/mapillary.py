...
from .sequential import extract_sequences
...
class MapillaryDataModule(pl.LightningDataModule):
    dump_filename = "outputs_per_sequence.json"
    ...
    def prepare_data(self):
        ...
        for scene in self.cfg.scenes:
            dump_dir = osp.join(self.root, scene)
            assert fs.exists(osp.join(dump_dir, self.dump_filename)), dump_dir
            assert fs.exists(osp.join(dump_dir, self.cfg.tiles_filename)), dump_dir

            # Cache the folder of images locally to speed up reading
            local_dir = self.local_dir / scene
            images = local_dir / self.images_dirname
            if not images.exists():
                images_archive = local_dir / self.images_archive
                logger.info("Preparing scene %s.", scene)
                if not images_archive.exists():
                    ...
                    images_archive_remote = osp.join(dump_dir, self.images_archive)
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        for scene in self.cfg.scenes:
            ...
            self.tile_managers[scene] = TileManager.load(
                osp.join(dump_dir, self.cfg.tiles_filename)
            )
            ...
            with fsspec.open(osp.join(dump_dir, self.dump_filename), "r") as fp:
            ...
            self.image_dirs[scene] = self.local_dir / scene / self.images_dirname
    ...
    def pack_data(self):
        ...
        for stage, names in self.splits.items():
            ...
            for k in data:
                v = np.array(data[k], dtype=np.float32 if k in to_fp32 else None)
    ...
    def parse_splits(self, split_arg, names):
        if split_arg is None:
            ...
            with fsspec.open(osp.join(self.root, split_arg), "r") as fp:
                self.splits = json.load(fp)
            # subsample based on the given scenes
            self.splits = {
                k: [n for n in ns if n[0] in self.cfg.scenes]
                for k, ns in self.splits.items()
            }
        else:
    ...
    def sequence_dataset(self, stage: str, chunk_args: Dict = None):
        ...
        chunks = extract_sequences(self.dumps, keys, **chunk_args)
        chunk2idx = {
            (scene, seq, i): [key2idx[scene, seq, n] for n in ns]
            for scene in chunks
            for seq in chunks[scene]
            for i, ns in enumerate(chunks[scene][seq])
        }
        chunk_indices = torch.full((len(keys),), -1)
        for (_, _, chunk_index), idx in chunk2idx.items():
            chunk_indices[idx] = chunk_index
        self.data[stage]["chunk_index"] = chunk_indices
        dataset = self.dataset(stage)
        return dataset, chunk2idx
