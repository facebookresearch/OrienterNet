...
class TemplateSampler(torch.nn.Module):
    ...
    def forward(self, image_bev):
        ...
        kernels = grid_sample(image, grid, align_corners=False).reshape(b, n, c, h, w)
...
def fuse_gps(log_prob, uv_gps, ppm, sigma=10, gaussian=False):
    ...
    if gaussian:
        gps_log_prob = -1 / 2 * dist / (sigma * ppm) ** 2
    else:
        gps_log_prob = torch.where(dist < sigma**2, 1, -np.inf)
    # TODO: decompose x and y for more efficiency
