...
class Location2DRecall(torchmetrics.MeanMetric):
    def __init__(self, threshold, pixel_per_meter, key="xy_max", *args, **kwargs):
    ...
    def update(self, pred, data):
        error = location_error(pred[self.key], data["xy"], self.ppm)
...
class Location2DError(MeanMetricWithRecall):
    ...
    def update(self, pred, data):
        value = location_error(pred[self.key], data["xy"], self.ppm)
        if value.numel():
            self.value.append(value)


class LateralLongitudinalError(MeanMetricWithRecall):
    def __init__(self, pixel_per_meter, key="xy_max"):
        super().__init__()
        self.ppm = pixel_per_meter
        self.key = key
    ...
    def update(self, pred, data):
        ...
        shift = (pred[self.key] - data["xy"]) * yaw.new_tensor([-1, 1])
